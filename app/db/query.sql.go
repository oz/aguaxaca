...
RETURNING id, delivery_id, name, created_at
`
...
func (q *Queries) CreateSublocation(ctx context.Context, arg CreateSublocationParams) (Sublocation, error) {
	row := q.db.QueryRowContext(ctx, createSublocation, arg.DeliveryID, arg.Name)
	var i Sublocation
	err := row.Scan(
		&i.ID,
		&i.DeliveryID,
...
const getSublocationsByDeliveryID = `-- name: GetSublocationsByDeliveryID :many
SELECT id, delivery_id, name, created_at FROM sublocations
WHERE delivery_id = ?
ORDER BY name
`

func (q *Queries) GetSublocationsByDeliveryID(ctx context.Context, deliveryID int64) ([]Sublocation, error) {
	rows, err := q.db.QueryContext(ctx, getSublocationsByDeliveryID, deliveryID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Sublocation
	for rows.Next() {
		var i Sublocation
		if err := rows.Scan(
			&i.ID,
